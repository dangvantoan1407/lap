...
const checkEmail = () => {
    ...
    if (!isRequired(email)) {
        ...
        showError(emailEl, 'Email is not vaild.');
    } else {
        showSuccess(emailEl);
        valid = true;
    }
...
const checkConfirmPassword = () => {
    ...
    const checkConfirmPassword = confirmPasswordEl.value.trim();
    const password = passwordEl.value.trim();

    if (!isRequired(confirmPassword)) {
        showError(confirmPasswordEl, 'Please enter the password again');
    } else if (password !== confirmPasswordEl) {
        showSuccess(confirmPasswordEl, 'The password does not match');
    } else {
        showError(checkConfirmPassword);
        vaild = true;
    }
...
const showError = (input, mwessage) => {
...
form.addEventListener('submit', function (e) {
     ...
     // validate fields
     let isUsernameValid = checkUsernam(),
         isEmailValid = checkEmail(),
         isPasswordValid = checkPassword(),
         isConfirmPassworfValid = checkConfirmPassword();
    
     let isFormValid = isUsernameValid &&
     isEmailValid &&
     isPasswordValid &&
     isConfirmPassworfValid;
